...
procedure lab06_pruebas IS
  ...
  procedure red IS begin null; end red;
  procedure green IS begin null; end green;
  procedure white IS begin null; end white;
  procedure end_credits(aciertos, fallos: in Natural) IS begin New_Line;Put(aciertos,0);if aciertos/=1 then Put(" exitos, ");else Put(" exito, ");end if;Put(fallos,0);if fallos/=1 then Put(" fallos (");else Put(" fallo (");end if;if fallos/=0 then Put(aciertos*100/(aciertos+fallos), 0);Put("%).");else Put(aciertos*100/(aciertos+fallos), 0);Put("%");white;Put(").");end if;New_Line;if hay_exclamaciones then Put("Resultados con ! en el titulo pueden contener falsos negativos.");end if;end end_credits;
